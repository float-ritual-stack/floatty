...
**Also the one Solid-2-forward-compatible discipline** (verified against the 2.0 RC migration guide, 2026-08-29): Solid 2.0 splits effects into a tracking *compute* phase and a side-effecting *apply* phase, and **writing to a signal during the tracking phase throws in dev**. Isolating tracked reads from writes — `on(dep, (v) => setDerived(v))`, or deriving with `createMemo` instead of writing in an effect — is therefore both the 1.9 dependency-leak fix AND the shape 2.0 requires. It is the *only* pre-adoptable Solid-2 pattern: `onCleanup` / `onMount` / `on()` are all **removed** in 2.0 and their replacements (returned-cleanup, `onSettled`, two-arg `createEffect`) are NOT valid in 1.9.x (a returned function becomes the next run's accumulator, not a cleanup — it leaks). Don't pre-adopt those; migrate them in one pass at 2.0-stable. See `performance-levers.md` §"review heuristics" for why "migrate to Solid 2.0" is not a performance fix.
