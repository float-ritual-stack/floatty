...
- **Backlinks drawer — per-pane, resizable, persisted** ([[PR #411]], [[FLO-440]] — `BacklinkDrawer.tsx`, `OutlinerPane.tsx`, `lib/drawerLayout.ts`, `lib/backlinkScope.ts`, `usePaneStore.ts`, `useWorkspacePersistence.ts`): a bottom drawer housed in every outliner pane with a keyboard-operable grip (`role=separator`, ±16/±64/Home/End), height clamped to the pane, open state and height persisted per pane and restored across restarts. Scope stacks focal-block groups above the always-present page group, so the drawer answers "what points at what I'm looking at" whether that's the zoomed page or the focused block.
- **Reference rows with crumb, content, child peek, and expand-in-place** ([[PR #412]], [[FLO-440]] — `BlockRefList.tsx`, `lib/backlinkRows.ts`): each row shows the source's ancestor chain as clickable segments (canonical page/section labels via `getPageTitle`/`getSectionKey`), the source line wrapped to two lines instead of mid-truncated, and the first child as a preview — because the reference is so often on a parent whose payload is the child. `▸` expands the row into a D4 slice (ancestor → highlighted source → up to 4 children); clicking a crumb segment re-roots the slice at that ancestor. `▸ all` / `▾ all` expands or collapses every visible row.
- **Facets, filter, sort, and navigate** ([[PR #412]], [[FLO-440]] — `BlockRefList.tsx`, `lib/backlinkRows.ts`): page / link / marker chips with counts; click includes, shift+click excludes; chips are **contextual** — counts are computed over the currently-filtered rows and any option that would return zero results is omitted, so narrowing never dead-ends. Free-text filter over content, child preview, page name, and crumb. Sort by updated / created / page, primary-only flip. `→` calls `resolveSameTabLink` → `navigateToBlock` — the existing funnel, zero new navigation code, ⌘L pane links honoured.
- **Churn clustering — revisions fold behind one row** ([[PR #413]], [[FLO-440]] — `lib/backlinkRows.ts` `clusterChurn`, `BlockRefList.tsx`): same page + within 1h + bigram-Dice ≥ 0.5 over prose-only text (wikilinks, marker pills, clock-times stripped) collapses into one cluster fronted by the latest revision with a `⊟ N rev` chip that unstacks on click; an explicit `supersedes::` marker overrides the heuristic. Cluster rank is its earliest-sorted member, never a sum, so folding can't reorder the list.
- **`⟲n` inbound-count chips on blocks** ([[PR #413]], [[FLO-440]] — `BlockItem.tsx`, `index.css`): any block with inbound references shows its count inline after the content; clicking focuses the block and opens the drawer scoped to it. Reads from the shared index, so it's one memo per block, no per-block scan.
- **Canonical reverse index** ([[PR #410]], [[FLO-440]] — `lib/backlinkIndex.ts`, `WorkspaceContext.tsx`, `docs/design/2026-08-11-backlinks-drawer.md`): a single `observeDeep`-driven, rAF-coalesced target→sources index created once in `WorkspaceProvider` and exposed as `backlinks()`; `referencing(key)` is the one read every surface above uses. Ambiguous page-name targets are tracked separately rather than silently picking one. Design doc (D1–D13, U1–U5) lands alongside.

### 🐛 Fixes

- **"failed to load outline" / "Server not running" on large outlines** ([[PR #411]], [[FLO-440]] loop — `src-tauri/src/server.rs`): `wait_for_server_health` was 30 probes × 100ms, but a connection-refused probe returns instantly, so the whole ladder collapsed to ~3s — and a 42MB Y.Doc replays for ~4s before the server ever binds. Boot gave up on a server that was seconds from healthy. Now a 30s wall-clock deadline (`wait_for_health_until`, 250ms interval, deadline checked before every probe and sleep). Two regression tests: `health_wait_survives_slow_bind`, `health_wait_respects_deadline_when_nothing_binds`.
- **App froze solid after opening the drawer on a busy page** ([[PR #412]], [[PR #413]] — `BlockRefList.tsx`): rendering rebuilt row objects through reference-keyed `<For>` remounted every row on each index rebuild, and WebKit recomputed event regions for the whole tree each time (`updateEventRegionsRecursive`, WebContent at 97.8% CPU, UI unresponsive). Every list level — groups, clusters, rows, chips, crumb segments, slice lines — now uses `<Key>` by id (Fatal Mistake #2, one level deeper than the first fix caught).
- **`rebuild.sh` no longer exits 1 on every remote-mode deploy** ([[PR #408]], [[FLO-762]] — `scripts/rebuild.sh`): the final health check polled `127.0.0.1:$PORT` unconditionally, which can never succeed when `remote_server_url` is set. Now branches on mode: local polls the app's own server; remote verifies the GUI launched and reports the remote it connects to. Hit live during the v0.26.2 deploy.

### ♻️ Refactors

- **`filter::` and the dead-arch siblings are gone** ([[PR #409]], [[FLO-890]] — deleted `FilterBlockDisplay.tsx`, `filterParser.ts`, `docs/guides/FILTER.md`, `src/lib/outliner/`, `doors/render-test/`; `blockTypes.ts`, `inlineParser.ts`, `markdownExport.ts`, `block.rs`, regenerated `BlockType` bindings): the `filter::` view, parser, inline tokens, and ~27 CSS rules; the declared-but-dead `web::`/`link::`/`dispatch::` prefixes; the orphan second `BlockType` union; and the Rust `Filter`/`Web`/`Dispatch` variants. `deploy-doors.sh` gains an empty-array guard for macOS bash 3.2 under `set -u`. **Behavior change**: any existing `filter::` block now renders as plain text — the projection-surfaces spine ([[FLO-947]]) is the successor, not a rewrite of this.
- **`LinkedReferences` retired** ([[PR #413]], [[FLO-440]] — deleted `LinkedReferences.tsx`, `findBacklinks`, `extractAllWikilinkTargets` import in `useBacklinkNavigation.ts`, its tests and CSS; `Outliner.tsx` mount): the drawer is the single backlinks surface. Nothing else consumed the old scan.

### 🧪 Tests

- New suites: `backlinkRows.test.ts` (33 — crumbs, truncation, facets, contextual counts, sort, slice, churn norm/Dice/clustering), `BlockRefList.test.tsx` (11 — facet include/exclude, filter, navigate, inert rows, expand-in-place, crumb re-root, churn unstack, expand-all), `BacklinkDrawer.test.tsx` (8 — grip keyboard/pointer, ARIA, nav wiring), `backlinkScope.test.ts`, `drawerLayout.test.ts`; `usePaneStore.test.ts` +5 (drawer state, hydrate guard), `BlockItem.test.tsx` +2 (`⟲n` chip), `server.rs` +2 (health-wait). All fixtures synthetic (`Demo Alice`, `00000000-0000-4000-8000-…`). Vitest 1851 passing | 2 skipped; Rust workspace 628.
...
- **Three floatty-specific directives** ([[PR #308]] / `082771b` — `packages/render-catalog/src/directives.ts`) — `$projectColor(name)` resolves a project namespace (e.g. `"floatty"`, `"rangle/pharmacy"`) to its canonical hex via `STREAM_PROJECT_COLORS`; `$ctxColor(mode)` resolves a ctx:: mode (e.g. `"debugging"`, `"session-archaeology"`) via `STREAM_MODE_COLORS`; `$wikilink(target)` is currently an identity transform that declares semantic intent (future enrichment slot for backlink counts / status badges). Unknown values resolve to `UNKNOWN_COLOR` (`#888`). 14 new unit tests cover resolution + composition with `$concat` + the rejection of non-directive object shapes.
- **`floattyDirectives` bundle** ([[PR #308]] / `082771b` — `packages/render-catalog/src/directives.ts`) — single export combining `standardDirectives` from `@json-render/directives` (7 entries) + the 3 floatty-specific ones. Pass once to `<JSONUIProvider directives={...}>` and `bbsCatalog.prompt({ directives })`; each directive's `description` lands in the agent's "CUSTOM DYNAMIC VALUES" prompt section.
- **`directiveOr<T>(schema)` helper** ([[PR #308]] / `082771b`, `faca23b` — `packages/render-catalog/src/directives.ts`) — two-layer-contract Zod wrapper: wire-time validation accepts the inner schema OR any record with at least one `$`-prefixed key (via `.refine()` — not regex on record keys, which Zod 4 enforces per-key and would reject valid multi-arg directives like `{ $math: "add", a: 1, b: 2 }`). Type-asserts return as `T` because renderers receive resolved values post-`resolvePropValue`. Non-directive objects like `{ color: "cyan" }` reject loudly.
- **Turbo cache wins** ([[PR #308]] / `09eb330` — `turbo.json`, root `package.json`) — `outputs` declared on `build` (`dist/**`, `.next/**`, `!.next/cache/**`); `lint` / `test` / `build` route through the existing `transit` task so workspace-dep source changes invalidate downstream caches correctly; new root `pnpm typecheck` script. Warm-cache results: build 8.2s → 23ms, typecheck 17ms, lint 19ms, test 17ms (all FULL TURBO).

### ♻️ Refactors

- **Render-door provider stack collapsed to `JSONUIProvider`** ([[PR #308]] / `ecd6e92` — `packages/render-door/src/render.tsx`) — switched from four individual primitives (`StateProvider` + `ActionProvider` + `VisibilityProvider` + `ValidationProvider`) to one combined `<JSONUIProvider>`. Required because `DirectivesContext` is `const` (not exported) in `@json-render/solid`; the only public surfaces that register directives are `JSONUIProvider` and `createRenderer`. Hoisted `initialState` + `onStateChange` into `RenderViewInner`'s props so the combined provider lives where the action handlers are constructed. `registry` passed to `JSONUIProvider` per the 0.18 API change.
- **`STREAM_PROJECT_COLORS` / `STREAM_MODE_COLORS` moved to `@float/render-catalog`** ([[PR #308]] / `082771b` — `packages/render-catalog/src/colors.ts`, `packages/render-door/src/components.tsx`) — single source of truth so the directive resolvers and the existing `ContextStream` component never drift. Render-door imports from render-catalog; the 3 inline `?? '#888'` fallback sites swapped to `?? UNKNOWN_COLOR` constant.
- **`shared.ts` prop slots widened via `directiveOr`** ([[PR #308]] / `582623c` — `packages/render-catalog/src/components/shared.ts`) — 18 prop slots accept directive-shaped objects in addition to plain strings: 16 `color` slots (StatPill, TimelineEvent, LinkGraph node, ActivityHeatmap, Hero cover, CardCover cover/properties, plus 8 more), 2 `href` slots (Hero actions, CardCover card-link), 2 `target` slots (GapItem, WikilinkChip — StepIndicator + EnrichedStepCard kept narrow since those are HTTP fetch targets, not wikilinks).
- **Outline-explorer `ExplorerRenderer` default-injects directives** ([[PR #308]] / `2260bfd` — `apps/outline-explorer/src/lib/catalog/explorer-renderer.tsx`) — wraps `createRenderer()` output to pass `directives={floattyDirectives}` by default. All three call sites (message-bubble, block-content, mcp app main) pick it up transparently. Symmetric with render-door.
- **Workspace `@json-render/*` bumped to ^0.19.0** ([[PR #308]] / `054ecc7` — 6 package.jsons) — `@json-render/{core,solid,react,mcp,ink,directives,devtools-solid}` to `^0.19.0` across render-catalog, render-door, apps/floatty, apps/outline-explorer, apps/render-reference, apps/ink-chat. Ink-chat jumped 0.17 → 0.19 — picks up the Zod 4 record/default/literal fix from 0.18. `@json-render/ink` doesn't implement directive resolution (the 0.19 changelog explicitly scopes directives to React/Vue/Svelte/Solid), so ink-chat just rides the version bump for workspace consistency.

### 🐛 Fixes

- **`directiveOr` rejects non-directive objects** ([[PR #308]] / `faca23b` — `packages/render-catalog/src/directives.ts`) — original union `z.record(z.string(), z.unknown())` accepted any record, so `{ color: "cyan" }` would pass wire-time validation but resolve to `undefined` at render time (silent failure). Constrained to records containing at least one `$`-prefixed key via `.refine()`. Caught a subtle Zod 4 behaviour: `z.record(z.string().regex(/^\$/), ...)` rejects multi-key directives because the regex applies per-key. (CodeRabbit Critical + Greptile P2).
- **`safeInitialState` guards non-object spec.state** ([[PR #308]] / `faca23b` — `packages/render-door/src/render.tsx`) — `spec()?.state || {}` would pass truthy non-object values (legacy fixtures with `state: ""` or `state: 1`) through to `RenderViewInner`. Helper coerces to `{}` unless input is a plain non-array object. (CodeRabbit Major).
- **Test count assertion replaced with subset check** ([[PR #308]] / `faca23b` — `packages/render-catalog/src/directives.test.ts`) — `floattyDirectives.length === 10` would break if `@json-render/directives` adds or removes entries in a future 0.19.x patch. Now asserts exactly 3 floatty-specific directives + name-checks a few bundled `standardDirectives`. (Greptile P2).
- **Color-prompt redundancy resolved** ([[PR #308]] / `faca23b` — `packages/render-door/src/render.tsx`) — `bbsCatalog.prompt({ directives })` now teaches the agent about `$projectColor` / `$ctxColor`, but the immediately-following `Colors:` line still listed raw hex. Reworded to prefer the directive form when a `[project::X]` or `[mode::Y]` semantic applies; raw hex labelled as ad-hoc fallback. (Greptile P2).
- **README MD038 backtick spacing** ([[PR #308]] / `faca23b` — `packages/render-catalog/README.md`) — markdownlint-cli2 flagged inline code-span composition on the new directives table row. Each directive name now in its own backtick span; compound JSX-shaped token replaced with plain prose. (CodeRabbit Minor).

### 🧪 Tests

- **14 new directive tests** ([[PR #308]] / `082771b`, `faca23b` — `packages/render-catalog/src/directives.test.ts`) — `$projectColor` / `$ctxColor` / `$wikilink` resolution including unknown fallbacks; registry contains each directive; composition with `$concat` (e.g. `{ $concat: ["see [[", { $wikilink: "FLO-679" }, "]] for details"] }` → `"see [[FLO-679]] for details"`); `directiveOr` accepts strings + directive objects + numeric base schemas; rejects bare primitives + non-directive objects. Total workspace: 1416/1416 passing (15 render-catalog + 93 render-door + 1308 float-pty).

### 📝 Docs

- **5 doc files refreshed for 0.19 + turbo improvements** ([[PR #308]] / `f8cd941` — `CLAUDE.md`, `apps/ink-chat/CLAUDE.md`, `apps/outline-explorer/CLAUDE.md`, `apps/render-reference/CLAUDE.md`, `packages/render-catalog/README.md`) — version-string bumps (0.17/0.18 → 0.19); root `CLAUDE.md` commands table updated with `pnpm typecheck` / `pnpm test` / `pnpm build` (turbo-orchestrated) and the FULL TURBO note; `render-catalog/README.md` documents the new `floattyDirectives` / `directiveOr` / `STREAM_*` exports.
