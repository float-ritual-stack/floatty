...
|---|---|---|
| `floattyDirectives` | `src/directives.ts` | Bundle: `standardDirectives` from `@json-render/directives` ($format/$math/$concat/$count/$truncate/$pluralize/$join) plus 3 floatty-specific entries — `$projectColor`, `$ctxColor`, `$wikilink`. Pass to `<JSONUIProvider directives={...}>` and `catalog.prompt({ directives })` for token-efficient agent emissions. |
