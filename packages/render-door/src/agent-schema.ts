...
export function buildAgentSystemPrompt(): string {
  ...
  return [
    ...
    '',
    'IF THE USER ASKS FOR MULTIPLE BLOCKS / SECTIONS / SPLIT-INTO-PIECES:',
    '- DEFAULT: Compose ONE spec with multiple sub-elements via Stack / Group / Tabs containers',
    '  in the catalog. ONE block, ONE rendered view, multiple visual sections inside it.',
    '  This is almost always what the user wants — they want to SEE multiple sections,',
    '  not necessarily have multiple separate blocks in the outline tree.',
    '',
    '- ONLY IF the user EXPLICITLY says "as separate blocks" / "as siblings" / "individual blocks":',
    '  Use the floatty REST API or MCP tools you have access to (e.g. add_block) to write',
    '  each section as a separate block, AND USE THE PROJECTION-CONTRACT SHAPE:',
    '',
    '    POST /api/v1/blocks',
    '    {',
    '      "content": "Section Title (semantic, ≤80 chars)",',
    '      "outputType": "door",',
    '      "output": {',
    '        "kind": "view",',
    '        "doorId": "render",',
    '        "schema": 1,',
    '        "data": { "spec": <section spec>, "title": "Section Title", "generatedVia": "agent" }',
    '      },',
    '      "outputStatus": "complete",',
    '      "afterId": "<sibling block uuid or prefix>"',
    '    }',
    '',
    '  The MCP add_block tool accepts the same `output` / `outputType` / `outputStatus`',
    '  fields. Use them.',
    '',
    'DO NOT WRITE BLOCKS WITH `content: "render:: {full JSON spec}"`:',
    '- That works (auto-execute will project it) but produces a degraded UX:',
    '  the contentEditable shows the raw JSON wall, and any reconnect / sync',
    '  event briefly unmounts the rendered view exposing the JSON. Use the',
    '  projection-contract shape above — content = semantic title, output = envelope.',
    '',
    LAYOUT_PATTERNS,
  ].join('\n');
}
