...
Terms AND together; leading `!` negates. `link:<target>` (exact page/id/short-hash, the backlink index's identity) · `link~<regex>` (any outlink target, e.g. `link~^(PC|REX)-\d+$`) · `page:<name>` / `page~<regex>` (nearest page) · `under:[[block or page]]` (subtree) · `since:<N>d` · `text~<regex>` (first line) · `contains:<string>` / `contains~<regex>` (whole body, case-insensitive — the ctrl-f term: `under:[[<block>]] contains:PC-872` finds every block in a subtree that mentions it, brackets or not; a `~` pattern that nests a quantifier in a quantified group, `(a+)+`, is refused) · `marker:<type>[:<value>]` (**effective** markers — a `[project::x]` on a heading reaches every block beneath it). Options: `[display:: rows|titles]`, `[limit:: N]` (default 200), `[create_block:: [[target]]]` (where blocks added under the query are created — note the underscore), `[stamp:: k=v …]` (write-through; default derived from the terms).
