 ...
 *
 * Extracted from src/components/views/DailyView.tsx into plugin form.
 */

import { For, Show } from 'solid-js';
import type { DailyNoteData, TimelogEntry, PrInfo, ScatteredThought } from './dailyHandler';
import type { OutputViewProps } from '../../lib/plugins/types';
...
/**
 * Main daily view — renders timeline + stats + scattered thoughts.
 * Conforms to OutputViewProps for the plugin view registry.
 */
export function DailyView(props: OutputViewProps) {
  const data = () => props.data as DailyNoteData;

  // Defensive fallbacks for optional fields
  const stats = () => data().stats || { sessions: 0, hours: '—', prs: 0 };
    ...
    <div class="daily-view">
      ...
      <div class="daily-header">
        <div class="daily-date-info">
          <div class="daily-date">{data().date}</div>
      ...
      {/* Scattered thoughts */}
      <Show when={data().scattered_thoughts?.length > 0}>
        <div class="daily-notes">
          <h2>Notes</h2>
          <For each={data().scattered_thoughts}>
      ...
      {/* Empty state */}
      <Show when={timelogs().length === 0 && (!data().scattered_thoughts || data().scattered_thoughts.length === 0)}>
        <div class="daily-empty">No timelogs or notes found for {data().date}</div>
      </Show>
    </div>
  );
}
...
/**
 * Error view for daily:: failures.
 * Conforms to OutputViewProps for the plugin view registry.
 */
export function DailyErrorView(props: OutputViewProps) {
  const error = () => {
    const d = props.data as { error?: string };
  ...
  return (
    <div class="daily-error">
      ...
      <span class="daily-error-text">{error()}</span>
    </div>
  );
}
